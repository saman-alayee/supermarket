...
const selectedOptions = computed(() =>
  props.options.filter((option) => props.modelValue.includes(String(option.value)))
);
...
const menuId = `app-multi-select-${Math.random().toString(36).slice(2, 9)}`;
...
<template>
  <div ref="triggerRef" class="relative w-full">
    ...
    <button
      ...
      :disabled="disabled"
      :class="[
        'input-field flex items-center justify-between gap-2 text-start transition-all min-h-[42px]',
        ...
        !selectedOptions.length ? 'text-gray-400' : 'text-gray-800',
      ]"
      @click.stop="toggle"
    >
    ...
    <div v-if="selectedOptions.length" class="mt-2 flex flex-wrap gap-1.5">
      <button
        ...
        @click="removeValue(String(option.value))"
      >
        {{ option.label }}
        <AppIcon name="lucide:x" size="xs" />
      </button>
    </div>
              ...
              >
                <span class="flex items-center gap-2 min-w-0 truncate">
                  <AppIcon
                    ...
                    :name="option.icon"
                    size="sm"
                    :class="isSelected(option.value) ? 'text-primary-600' : 'text-gray-400'"
                  />
                  {{ option.label }}
                </span>
                <AppIcon
